...
class CameraActivity : AppCompatActivity() {
    ...
    private lateinit var btnFlipCamera: MaterialButton
    ...
    private fun setupUI() {
        val color = if (type == "masuk") R.color.blue_bold else R.color.orange_bold

        btnCapture.backgroundTintList = ContextCompat.getColorStateList(this, color)
        ...
        btnFlipCamera.strokeColor = ContextCompat.getColorStateList(this, color)
        btnFlipCamera.iconTint = ContextCompat.getColorStateList(this, color)
    }
        ...
        MediaManager.get().upload(photoFile.path)
            ...
            .callback(object : UploadCallback {
                ...
                override fun onSuccess(requestId: String?, resultData: MutableMap<Any?, Any?>?) {
                    ...
                    // Jika Tukar Shift pada absensi MASUK → simpan sebagai Pending
                    // Status sementara di Firebase = "Alpa" agar rekap menganggapnya Alpa
                    // approvalStatus = "pending" menandai bahwa ini menunggu persetujuan admin
                    val isTukarShiftMasuk = tukarShift && type == "masuk"

                    val finalStatus = if (isTukarShiftMasuk) {
                        // Simpan status yang akan berlaku jika disetujui (untuk referensi admin)
                        TimeUtils.determineStatus(
                            selectedStatus = status ?: "Hadir",
                    ...
                    val data = hashMapOf(
                        // Jika pending: status di Firebase = "Alpa" sementara
                        "status" to if (isTukarShiftMasuk) "Alpa" else finalStatus,
                        ...
                        // Field baru untuk sistem approval Tukar Shift
                        "approvalStatus" to if (isTukarShiftMasuk) "pending" else "",
                        "statusJikaDisetujui" to if (isTukarShiftMasuk) finalStatus else ""
                    )
                    ...
                    FirebaseFirestore.getInstance()
                        ...
                        .addOnSuccessListener {
                            ...
                            val msg = if (isTukarShiftMasuk)
                                "Pengajuan Tukar Shift berhasil, menunggu persetujuan Admin"
                            else
                                "Absen berhasil"
