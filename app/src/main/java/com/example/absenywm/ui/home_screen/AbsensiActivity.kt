...
class AbsensiActivity : AppCompatActivity() {
    ...
    private fun goToCamera() {
        ...
        // Validasi Tukar Shift
        if (tukarShift) {

            val selectedShift = dropdownShift.text.toString().trim()

            if (selectedShift.isEmpty()) {
                layoutShift.error = "Silakan pilih shift pengganti"
                dropdownShift.requestFocus()
                return
            }
        ...
        val selectedShift = if (tukarShift) {
            dropdownShift.text.toString().trim()
        } else {
            null
        }
